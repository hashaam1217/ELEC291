/*-------------------------------------------------------------------------
  ...
  C51icode.c - intermediate code generation etc.
...
newiTemp (char *s)
{
    ...
    strncpyz (itmp->rname, itmp->name, C51_NAME_MAX);
    itmp->isitmp = 1;
    ...
    case SYMBOL:
        if (op->operand.symOperand->isitmp)
		{
            op->operand.symOperand->etype =
                getSpec (op->operand.symOperand->type = copyLinkChain (type));
			if (IS_SPEC (op->operand.symOperand->type)) // JCF: Fix for bug 3166208
			{
				SPEC_SCLS (op->operand.symOperand->etype) = S_REGISTER;
				SPEC_OCLS (op->operand.symOperand->etype) = reg;
			}
 	  	}
        else
            werror (E_INTERNAL_ERROR, __FILE__, __LINE__,
...
#if 0
        // disabled because C51 will make all union fields volatile
    ...
    if (IS_PTR(type))   // to a pointer
    {
        if (!IS_PTR(optype) && !IS_FUNC(optype) && !IS_AGGREGATE(optype))   // from a non pointer
        {
            if (IS_INTEGRAL(optype))
            {
                ...
                {
                    if (port->s.gptr_size > port->s.fptr_size && IS_GENPTR(type) && !IS_FUNCPTR (type))
                    {
...
geniCodeAdd (operand * left, operand * right, RESULT_TYPE resultType, int lvl)
{
	...
	if (IS_PTR (ltype) && right->isLiteral )
	{
		//printf("ptr + literal\n");
	}
	...
	/* JCF: This line was the main source of grief for bugs 3027957, 3037889, 3153956, and who know
	which others.  Removing it all together doesn't seem to affect the generated assembly*/
    if (!IS_PTR (ltype) && !IS_PTR (rtype) && IS_LITERAL (letype) && IS_LITERAL (retype)
            && left->isLiteral && right->isLiteral)
