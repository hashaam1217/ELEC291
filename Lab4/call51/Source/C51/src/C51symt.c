/*-------------------------------------------------------------------------
  C51symt.c - Code file for Symbols table related structures and MACRO's.
        ...
        int checkType)
{
    ...
    if (strlen(sname)>C51_SYMNAME_MAX)
    {
        werror (W_SYMBOL_NAME_TOO_LONG, C51_SYMNAME_MAX);
        sname[C51_SYMNAME_MAX]='\0';
    }
...
addDecl (symbol * sym, int type, sym_link * p)
{
    ...
    if (getenv("C51_DEBUG_FUNCTION_POINTERS"))
        fprintf (stderr, "C51symt.c:addDecl(%s,%d,%p)\n", sym->name, type, p);
...
{
    static int gCount = 0;
    static char gname[C51_NAME_MAX + 1];
...
checkStructFlexArray (symbol *sym, sym_link *p)
{
    ...
    if (IS_ARRAY (p))
    {
        ...
        if (!DCL_ELEM(p))
        {
            //if (!options.std_c99) JCF: Anoying and useless warning.
             ...
             RESULT_TYPE resultType, int op)
{
    ...
    /* C51's sign promotion:
       ...
       C51 tries hard to avoid promotion to int and does 8 bit calculation as
...
compareTypeExact (sym_link * dest, sym_link * src, int level)
{
    ...
    if ((srcScls == S_FIXED || srcScls == S_AUTO) && port->mem.default_globl_map == xdata
			&& (destScls == S_XDATA) && (level <= 0) )
	{
        srcScls = S_XDATA;
	}
...
processFuncArgs (symbol * func)
{
    ...
    if (getenv("C51_DEBUG_FUNCTION_POINTERS"))
        fprintf (stderr, "C51symt.c:processFuncArgs(%s)\n", func->name);
