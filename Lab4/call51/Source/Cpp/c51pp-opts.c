...
/* Holds switches parsed by c51pp_common_handle_option (), but whose
   handling is deferred to c51pp_common_post_options ().  */
...
/* Complain that switch CODE expects an argument but none was
   ...
   one.  */
bool
c51pp_common_missing_argument (const char *opt, size_t code)
{
  switch (code)
    {
    default:
...
unsigned int
c51pp_common_init_options (unsigned int argc, const char **argv ATTRIBUTE_UNUSED)
{
...
int
c51pp_common_handle_option (size_t scode, const char *arg, int value)
{
  ...
  switch (code)
...
#if 0 // pch not supported by c51pp
    ...
    case OPT_obj_ext_:
      ...
      /* We need to handle the -pedantic switches here, rather than in
         c51pp_common_post_options, so that a subsequent -Wno-endif-labels
         is not overridden.  */
...
bool
c51pp_common_post_options (const char **pfilename)
{
  struct cpp_callbacks *cb;
