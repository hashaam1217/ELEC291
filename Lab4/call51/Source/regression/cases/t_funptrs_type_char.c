...
typedef void (*ONEARGFUNPTR)(char) REENTRANT;
typedef long int (*FOURARGFUNPTR)(char, char, long int, long int) REENTRANT;
typedef char (*TYPEFUNPTR)(char, char) REENTRANT;
...
void
callViaPtr2(ONEARGFUNPTR fptr, char arg)
{
  (*fptr)(arg);
}
...
void
callViaPtr2Ansi(ONEARGFUNPTR fptr, char arg)
{
  fptr(arg);
}
...
char f_ret(char arg1, char arg2) REENTRANT
{
  char local;
  local = !arg1;
  return (local & arg2);
...
__getSuiteName(void)
{
  return "cases\\t_funptrs_type_char";
}
