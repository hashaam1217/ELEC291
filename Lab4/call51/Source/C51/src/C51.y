/*-----------------------------------------------------------------------
  ...
  C51.y - parser definition file for c51 :
...
function_declarator2
   ...
   | declarator2 '('            { NestLevel++ ; STACK_PUSH(blockNum, currBlockno); currBlockno = ++blockNo; }
                     parameter_type_list ')'
         {
             sym_link *funcType;
...
abstract_declarator2
   ...
   | abstract_declarator2 '(' { NestLevel++ ; STACK_PUSH(blockNum, currBlockno); currBlockno = ++blockNo; } parameter_type_list ')' {
       sym_link *p=newLink(DECLARATOR);
       DCL_TYPE(p) = FUNCTION;
...
end_block   : '}'     { NestLevel--; currBlockno = STACK_POP(blockNum); EmptyIF=0;} //  'if(something) {}' generates no warning!
            ;
   ...
   : IF '(' expr ')' { seqPointNo++;} statement else_statement
                           {
                              ...
                              $$->filename = $3->filename;
                              noLineno--;
							  if (EmptyIF==1) werrorfl($3->filename, $3->lineno, W_EMPTY_IF_STATEMENT);
                           }
   | SWITCH '(' expr ')'   {
                              ast *ex ;
