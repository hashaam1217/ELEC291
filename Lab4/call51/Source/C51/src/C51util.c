...
{
	static char buid_str[0x100];

	sprintf(buid_str, "%04d", revison_number);
...
/*-----------------------------------------------------------------*/
/* getBuildEnvironment - return environment used to build C51     */
/*-----------------------------------------------------------------*/
...
size_t C51snprintf(char *dst, size_t n, const char *fmt, ...)
{
    va_list args;
    int len;
