...
static unsigned long kmask[]={0x0000,
...
int IF_Count;
int force_EOF;
int pound_warning_enable=1; // Just after adding this feature I found two bugs in old code!
extern int linetest;
...
two_op1:
	...
	| A ',' data8
		{
			if(($3.val.k==0) && pass1 && pound_warning_enable)
			{
				if(ObjectFile==0) warning("maybe '#' is missing with constant expression.");
			}
			set_md($$.mode,1);
			set_ov($$.mode,0);
			set_sz($$.mode,1);
