/*-------------------------------------------------------------------------
  ...
  C51glue.c - glues everything we have done together into one file.
...
char * aopLiteralLong (value * val, int offset, int size)
{
	...
	/* This is not solving 3027957 but generating more bugs instead*/
	if (IS_GENPTR (val->type) && !IS_VOID (val->type->next) &&
 	   v && (offset >= FPTRSIZE) && (GPTRSIZE > FPTRSIZE))
	{
		//werror (E_CANNOT_USE_GENERIC_POINTER, val->name ? val->name : "<null>", "<null>");
	}
    ...
    tsprintf (buffer, sizeof(buffer),"!immedbyte", fl.c[3 - offset]);
#else
    tsprintf (buffer, sizeof(buffer), "!immedbyte", fl.c[offset]);
#endif
    return Safe_strdup (buffer);
}
...
/* WARNING: this function is used both here and in gen.c           */
/*-----------------------------------------------------------------*/
...
// Same as above, but gets rid of the anoying '#' at the begining of the string
char * aopLiteral1 (value * val, int offset)
{
    return (aopLiteralLong (val, offset, 1) + 1);
}

/*-----------------------------------------------------------------*/
/* emitDebugSym - emit label for debug symbol                      */
/*-----------------------------------------------------------------*/
static void emitDebugSym (struct dbuf_s * oBuf, symbol * sym)
{
    if (!sym->level) /* global */
    {
...
static void emitRegularMap (memmap * map, bool addPublics, bool arFlag)
{
    ...
    if (addPublics)
    {
		...
		{
			if(strlen(options.overlay_to)>0)
			{
				dbuf_tprintf (&map->oBuf, "\t%s\tsegment data overlay\n", options.overlay_to);
				dbuf_tprintf (&map->oBuf, "\trseg\t%s\n", options.overlay_to);
			}
			else
			{
    ...
    for (sym = setFirstItem (map->syms); sym; sym = setNextItem (map->syms))
    {
        ...
        if (sym->ival && sym->level == 0)
        {
            if ((SPEC_OCLS (sym->etype) == xidata) && !SPEC_ABSA (sym->etype))
            {
				...
				newSym->used++; // All XINIT symbols need to be generated!
        ...
        /* if it has an absolute address then generate an equate for this no need to allocate space */
        if (SPEC_ABSA (sym->etype) && !sym->ival)
        {
            ...
            if (options.debug)
            {
                ...
                dbuf_printf (&map->oBuf, " %s 0x%02x \n", equ, SPEC_ADDR (sym->etype)); // SFR debug info generated here
            }

            if      (map == sfr)    equ = "DATA";
			else if (map == data)   equ = "DATA";
            else if (map == sfrbit) equ = "BIT";
            else if (map == bit)    equ = "BIT";
			else if (map == xdata)  equ = "XDATA";
			else if (map == idata)  equ = "IDATA";
  			else if (map == code)   equ = "CODE";
			if(map == xdata)
			{
				dbuf_printf (&map->oBuf, "\tXSEG at 0x%04x\n", SPEC_ADDR (sym->etype));
				dbuf_printf (&map->oBuf, "%s: ds %d\n", sym->rname, getSize (sym->type) + sym->flexArrayLength);
				dbuf_printf (&map->oBuf, "\tpublic %s\n", sym->rname);
				dbuf_printf (&map->oBuf, "\trseg R_XSEG\n");
			}
			else if(map == data)
			{
				dbuf_printf (&map->oBuf, "\tDSEG at 0x%04x\n", SPEC_ADDR (sym->etype));
				dbuf_printf (&map->oBuf, "%s: ds %d\n", sym->rname, getSize (sym->type) + sym->flexArrayLength);
				dbuf_printf (&map->oBuf, "\tpublic %s\n", sym->rname);
				dbuf_printf (&map->oBuf, "\trseg R_DSEG\n");
			}
			else if(map == idata)
			{
				dbuf_printf (&map->oBuf, "\tISEG at 0x%04x\n", SPEC_ADDR (sym->etype));
				dbuf_printf (&map->oBuf, "%s: ds %d\n", sym->rname, getSize (sym->type) + sym->flexArrayLength);
				dbuf_printf (&map->oBuf, "\tpublic %s\n", sym->rname);
				dbuf_printf (&map->oBuf, "\trseg R_ISEG\n");
			}
			else
			{
				// JCF: //dbuf_printf (&map->oBuf, "%s\t%s\t0x%04x\n", sym->rname, equ, SPEC_ADDR (sym->etype));
				if(SPEC_ADDR (sym->etype)<0x100)
					dbuf_printf (&map->oBuf, "%-15s %s 0x%02x\n", sym->rname, equ, SPEC_ADDR (sym->etype));
				else
					dbuf_printf (&map->oBuf, "%-15s %s 0x%04x\n", sym->rname, equ, SPEC_ADDR (sym->etype));
			}
        }
        else
        {
            ...
            {
                emitDebugSym (&map->oBuf, sym);
                dbuf_printf (&map->oBuf, " equ $\n"); // JCF: This may no be correct for A51.
            }
...
/*-----------------------------------------------------------------*/
/* printChar - formats and prints a character string with DB       */
/*-----------------------------------------------------------------*/
void printChar (struct dbuf_s * oBuf, char *s, int plen)
{
    int i;
    int len = plen;
    int pplen = 0;
...
/*-----------------------------------------------------------------*/
void _printPointerType (struct dbuf_s * oBuf, const char *name)
{
    if (options.model == MODEL_FLAT24)
    {
        if (port->little_endian)
            dbuf_printf (oBuf, "\tdb %s,(%s >> 8),(%s >> 16)", name, name, name);
        else
            dbuf_printf (oBuf, "\tdb (%s >> 16),(%s >> 8),%s", name, name, name);
    }
    else
    {
        if (port->little_endian)
            dbuf_printf (oBuf, "\tdb %s,(%s >> 8)", name, name);
        else
            dbuf_printf (oBuf, "\tdb (%s >> 8),%s", name, name);
    }
}
...
/* printPointerType - generates ival for pointer type              */
/*-----------------------------------------------------------------*/
void printPointerType (struct dbuf_s * oBuf, const char *name)
{
...
/*-----------------------------------------------------------------*/
void printGPointerType (struct dbuf_s * oBuf, const char *iname, const char *oname, int type)
{
...
void printIvalType (symbol *sym, sym_link * type, initList * ilist, struct dbuf_s * oBuf)
{
    ...
    case 1:
        ...
        else
        {
            ...
            {
                dbuf_tprintf (oBuf, "\tdb 0x%02x\t; %u%c",
                              (unsigned int)floatFromVal(val)&0xff, (unsigned int)floatFromVal (val), 0 );
            }
            else
            {
                dbuf_tprintf (oBuf, "\tdb 0x%02x\t; % d%c",
                              (unsigned int)floatFromVal(val)&0xff, (int)floatFromVal (val), 0 ); //Bug in dbug_tprintf? Need to append zero to string?
            }
    ...
    case 2:
        ...
        if (port->little_endian)
        {
            if (IS_UNSIGNED (val->type))
            {
                dbuf_printf (oBuf, "\tdb %s,%s\t; %u\n",
                             aopLiteral1(val, 0), aopLiteral1(val, 1),
                             (unsigned int)floatFromVal(val) );
            }
            else
            {
                dbuf_printf (oBuf, "\tdb %s,%s\t; % d\n",
                             aopLiteral1(val, 0), aopLiteral1(val, 1),
        ...
        {
            dbuf_printf (oBuf, "\tdb %s,%s\n", aopLiteral1(val, 1), aopLiteral1(val, 0));
        }
        break;
    ...
    case 4:
        ...
        else
        {
            if (port->little_endian)
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s,%s",
                             aopLiteral1(val, 0), aopLiteral1(val, 1),
                             aopLiteral1(val, 2), aopLiteral1(val, 3));
            }
            else
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s,%s",
                             aopLiteral1(val, 3), aopLiteral1(val, 2),
                             aopLiteral1(val, 1), aopLiteral1(val, 0));
            }
            if (IS_FLOAT (val->type))
            {
...
/*-----------------------------------------------------------------*/
int printIvalChar (symbol * sym, sym_link * type, initList * ilist, struct dbuf_s * oBuf, char *s, bool check)
{
    value *val;
                     ...
                     struct dbuf_s * oBuf, bool check)
{
    ...
    if (ilist)
    {
        ...
        /* by a string                      */
        if (IS_CHAR (type->next) && ( (ilist->type == INIT_NODE) || (ilist->type == INIT_DEEP)) )
        {
...
int printIvalCharPtr (symbol * sym, sym_link * type, value * val, struct dbuf_s * oBuf)
{
    ...
    if (val->name && strlen (val->name))
    {
        ...
        else if (size == GPTRSIZE)
        {
            ...
            if (val->sym)
			{
				val->sym->used++; // JCF: The symbol is used.
			}
        }
        else
        {
    ...
    else
    {
        ...
        case 3:
            ...
            if (port->little_endian)
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s\n",
                             aopLiteral1(val, 0), aopLiteral1(val, 1), aopLiteral1(val, 2));
            }
            else
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s\n",
                             aopLiteral1(val, 2), aopLiteral1(val, 1), aopLiteral1(val, 0));
            }
            break;
        ...
        case 4:
            ...
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s,%s\n",
                             aopLiteral1(val, 0), aopLiteral1(val, 1), aopLiteral1(val, 2), aopLiteral1(val, 3));
            }
            else
            {
                dbuf_printf (oBuf, "\tdb %s,%s,%s,%s\n",
                             aopLiteral1(val, 3), aopLiteral1(val, 2), aopLiteral1(val, 1), aopLiteral1(val, 0));
            }
            break;
...
void printIvalPtr (symbol * sym, sym_link * type, initList * ilist, struct dbuf_s * oBuf)
{
    ...
    if (IS_LITERAL (val->etype))
    {
        ...
        case 3: // how about '390??
            ...
            if (port->little_endian)
                dbuf_printf (oBuf, "\tdb %s,%s", aopLiteral1(val, 0), aopLiteral1(val, 1));
            else
                dbuf_printf (oBuf, "\tdb %s,%s", aopLiteral1(val, 1), aopLiteral1(val, 0));
            if (IS_GENPTR (val->type))
                dbuf_printf (oBuf, ",%s\n", aopLiteral1(val, 2));
...
/*-----------------------------------------------------------------*/
/* emitStaticSeg - emitcode for the static segment.  'Static' here */
/* is a bit misleading, as symbols can be public too.              */
/*-----------------------------------------------------------------*/
void emitStaticSeg (memmap * map, struct dbuf_s * oBuf)
{
    ...
    for (sym = setFirstItem (map->syms); sym; sym = setNextItem (map->syms))
    {
        // printf("sym:%s, used:%d\n", sym->name, sym->used); // JCF: debug symbol usage
        ...
        /* if it is static but not volatile and never used then do nothing */
        if( (sym->used==0) && IS_STATIC (sym->etype) && !IS_VOLATILE(sym->etype) )
        {
            continue;
        }
        ...
        else
        {
            ...
            if (sym->ival)
            {
                ...
                --noAlloc;
                /* if sym is a simple string and sym->ival is a string, we don't need it anymore */
...
void emitMaps (void)
{
    ...
    emitRegularMap (sfr, FALSE, FALSE); // EQUs don't need public definition
    emitRegularMap (sfrbit, FALSE, FALSE); // EQUs don't need public definition
    emitRegularMap (home, TRUE, FALSE);
    emitRegularMap (code, TRUE, FALSE);

    if (port->genXINIT)
    {
        dbuf_tprintf (&code->oBuf, "\n\t!area\n", xinit->sname);
        emitStaticSeg (xinit, &code->oBuf);
    }

    if (options.const_seg)
    {
		if(strcmp(options.const_seg, "R_CONST")!=0)
		{
			dbuf_tprintf (&code->oBuf, "\n\t%s\tsegment code", options.const_seg);
		}
        dbuf_tprintf (&code->oBuf, "\n\t!area\n", options.const_seg);
    }
    emitStaticSeg (statsg, &code->oBuf); // String constants live here
...
void createInterruptVect (struct dbuf_s *vBuf)
{
    ...
    else if (!IFFUNC_HASBODY(mainf->type))
    {
    ...
    else // If main is defined, print entry point
    {
        dbuf_printf (vBuf, "\tCSEG at 0x%04x\n", options.code_loc);
        //dbuf_printf (vBuf, "\t%cjmp\t__c51_gsinit_startup\n", options.acall_ajmp?'a':'l');
        dbuf_printf (vBuf, "\t%cjmp\t_crt0\n", options.acall_ajmp?'a':'l');
    }
...
initialComments (FILE * afile)
{
    ...
    fprintf (afile, "; Version " C51_VERSION_STR " #%s (%s) (%s)\n",
...
printPublics (FILE * afile)
{
	...
	{
		if (SPEC_ABSA (sym->etype) && !sym->ival) /* if it has an absolute address and no initializer */
        {
		}
		else if (IFFUNC_ISINLINE(sym->type))
		{
			//printf("No public declaration for inline function %s\n", sym->rname);
		}
        else
		{
...
static void emitOverlay (struct dbuf_s * aBuf)
{
            ...
            ovrset = setNextItem (ovrSetSets))
    {
                ...
                sym = setNextItem (ovrset))
        {
            ...
            {
                if (options.debug)
                    dbuf_printf (aBuf, " EQU 0x%04x\n", SPEC_ADDR (sym->etype)); // JCF DEBUG

                dbuf_printf (aBuf, "%s\tequ\t0x%04x\n", sym->rname, SPEC_ADDR (sym->etype));
            }
            else
            {
                int size = getSize(sym->type);
...
void glue (void)
{
    ...
    if (options.debug)	cdbStructBlock (0);
    ...
    /* print module name */
    tfprintf (asmFile, "$name %s\n", moduleName);

    fprintf (asmFile, "$optc51");
	...
	if(options.printf_float)
	{
		fprintf (asmFile, "$printf_float\n");
	}
    ...
    /* Create the areas for the register banks*/
    if (RegBankUsed[0] || RegBankUsed[1] || RegBankUsed[2] || RegBankUsed[3])
    {
        ...
        if (RegBankUsed[0])
        {
            fprintf (asmFile, "\trbank0 segment data overlay\n");
        }
        if (RegBankUsed[1] || options.parms_in_bank1)
        {
            fprintf (asmFile, "\trbank1 segment data overlay\n");
        }
        if (RegBankUsed[2])
        {
            fprintf (asmFile, "\trbank2 segment data overlay\n");
        }
        if (RegBankUsed[3])
        {
            fprintf (asmFile, "\trbank3 segment data overlay\n");
        }
    }
    ...
    if (BitBankUsed)
    {
        ...
        fprintf (asmFile, "%s", iComments2);
        fprintf (asmFile, "\trseg BIT_BANK\n");
        fprintf (asmFile, "bits:\n\tds 1\n");
        fprintf (asmFile, "\tb0 equ  bits.0 \n");
        fprintf (asmFile, "\tb1 equ  bits.1 \n");
        fprintf (asmFile, "\tb2 equ  bits.2 \n");
        fprintf (asmFile, "\tb3 equ  bits.3 \n");
        fprintf (asmFile, "\tb4 equ  bits.4 \n");
        fprintf (asmFile, "\tb5 equ  bits.5 \n");
        fprintf (asmFile, "\tb6 equ  bits.6 \n");
        fprintf (asmFile, "\tb7 equ  bits.7 \n");
    }
    ...
    if (mainf && IFFUNC_HASBODY (mainf->type) && options.useXstack)
    {
        ...
        fprintf (asmFile, "%s", iComments2);
		fprintf (asmFile, "\tR_XSTK    segment xdata\n");
        fprintf (asmFile, "\trseg R_XSTK\n"
                 "__start__xstack:\n\tds\t100H\n\n");
    }
    ...
    {
        fprintf (asmFile, "; Reset entry point and interrupt vectors\n");
    }
    else
    {
        fprintf (asmFile, "; Interrupt vectors\n");
    }
    fprintf (asmFile, "%s", iComments2);
    ...
    */
    ...
    fprintf (asmFile, "%s", iComments2);
    fprintf (asmFile, "; data variables initialization\n");
    fprintf (asmFile, "%s", iComments2);
    fprintf (asmFile, "\trseg R_DINIT\n");
    dbuf_write_and_destroy (&statsg->oBuf, asmFile);
    fprintf (asmFile, "\t; The linker places a 'ret' at the end of segment R_DINIT.\n");

    /* copy over code */
    fprintf (asmFile, "%s", iComments2);
    fprintf (asmFile, "; code\n");
    fprintf (asmFile, "%s", iComments2);
	if(strcmp(options.code_seg, "R_CSEG")!=0)
	{
		tfprintf (asmFile, "\t%s\tsegment code\n", options.code_seg);
	}
