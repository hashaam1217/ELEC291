/*----------------------------------------------------------------------
    C51val.c :- has routine to do all kinds of fun stuff with the
...
value *constVal (const char *s)
{
	...
	/*Is a negative number ever passed to constVal() ?*/
	if(s!=NULL)
	{
		if(s[0]=='-') printf("********** %s **********\n", s);
	}
...
copyValue (value * src)
{
    ...
    strncpyz (dest->name, src->name, C51_NAME_MAX);
...
valNot (value * val)
{
    ...
    /* c51 will hold this in an 'unsigned char' */
...
getNelements (sym_link * type, initList * ilist)
{
    ...
    if (IS_ARRAY (type) && IS_CHAR (type->next) && !ilist->next)
    {
        ...
        if (v && IS_ARRAY (v->type) && IS_CHAR (v->etype)) /* yep, it's a string */
        {
