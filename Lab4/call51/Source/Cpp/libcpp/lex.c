...
/* Skip an __asm ... __endasm block.  We find the end of the comment by
   ...
   otherwise.  */
static int
_c51pp_skip_asm_block (cpp_reader *pfile)
{
...
static void
_c51pp_save_asm (cpp_reader *pfile, cpp_token *token, const unsigned char *from, int is_asm)
{
          ...
          if (CPP_OPTION (pfile, preproc_asm) == 0)
            {
              ...
              result->type = CPP_ASM;
              _c51pp_skip_asm_block (pfile);
              /* Save the __asm block as a token in its own right.  */
              _c51pp_save_asm (pfile, result, comment_start, result->val.node.node == pfile->spec_nodes.n__asm);
            }
