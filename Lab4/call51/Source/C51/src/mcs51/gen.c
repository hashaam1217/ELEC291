...
/*-----------------------------------------------------------------*/
static void
aopPtrForSym (symbol * sym, bool accuse, signed char offset, asmop *aop)
{
  char * base;
...
aopForSym (iCode * ic, symbol * sym, bool result)
{
  ...
  emitcode ("mov", "dptr,#%s", sym->rname); // JCF
  aop->size = getSize (sym->type);
    ...
    case AOP_STK:
		{
			...
			aopPtrForSym (sym, FALSE, (signed char) (aop->size - 1), aop);
    ...
    case AOP_STK:
		{
			...
			emitcode ("mov", "b,r0");
			aopPtrForSym (aop->aopu.aop_sym, FALSE, 0, aop);
...
toBoolean (operand * oper)
{
	...
	//if (IS_FLOAT (type)) printf("Is a float!\n");
    ...
    else
    {
        while (size--)
        {
			if ( (IS_FLOAT (type)) && (size==0) )
			{
				pushedB = pushB ();
				emitcode ("mov", "b,%s", aopGet (oper, offset++, FALSE, FALSE));
				emitcode ("clr", "b.7 ; Clear the sign bit");
				emitcode ("orl", "a,b");
				popB (pushedB);
			}
			else
...
genNot (iCode * ic)
{
    ...
    emitcode ("cjne", "a,#0x01,L%06d?", tlbl->key + fc);
    emitLabel (tlbl);
    outBitC (IC_RESULT (ic));
...
genCpl (iCode * ic)
{
    ...
    if (AOP_TYPE (IC_RESULT (ic)) == AOP_CRY)
    {
        ...
        {
            emitcode ("cjne", "%s,#0xFF,L%06d?", l, tlbl->key + fc);
        }
        else
        {
            MOVA (l);
            emitcode ("cjne", "a,#0xFF,L%06d?", tlbl->key + fc);
        }
        emitLabel (tlbl);
        outBitC (IC_RESULT(ic));
        goto release;
    ...
    if (options.useXstack)
    {
        ...
        else if (count != 0)
        {
            if ((FUNC_REGBANK (currFunc->type) == 0) && optimize.codeSize)
            {
                ...
                if (mask & 0x100)
                    emitcode ("lcall", "__c51_xpush_regs_r0\t;(%s)", szRegs);
                else
                    emitcode ("lcall", "__c51_xpush_regs\t;(%s)", szRegs);
                lineCurr->isInline = 1;
            }
            else
            {
    ...
    if (options.useXstack)
    {
        ...
        else if (count != 0)
        {
            if ((FUNC_REGBANK (currFunc->type) == 0) && optimize.codeSize)
            {
                ...
                emitcode ("mov", "b,#0x%02X", mask & 0xFF);
                if (mask & 0x100)
                    emitcode ("lcall", "__c51_xpop_regs_bits\t;(%s)", szRegs);
                else
                    emitcode ("lcall", "__c51_xpop_regs\t;(%s)", szRegs);
                lineCurr->isInline = 1;
            }
            else
            {
...
assignResultValue (operand * oper, operand * func)
{
	...
	if (func && IS_BIT (getSpec (operandType (func)))) // Fix bug 3419769
    {
        outBitC (oper);
        return FALSE;
    }
...
//  bool restoreBank = FALSE;
    ...
    if (IFFUNC_ISBANKEDCALL (dtype) && !SPEC_STAT(getSpec(dtype)))
    {
        ...
        else
        {
            ...
            emitcode ("lcall", "__c51_banked_call");
        }
    }
    else
    {
...
//    restoreBank=TRUE;
    ...
    else
    {
        ...
        else if (_G.sendSet)
        {
            ...
            emitcode ("mov", "a,#L%06d?", (rlbl->key + fc));
            emitcode ("push", "acc");
            emitcode ("mov", "a,#(L%06d? >> 8)", (rlbl->key + fc));
            emitcode ("push", "acc");
        ...
        else /* the send set is empty */
        {
            ...
            /* make the call */
            emitcode ("lcall", "__c51_call_dptr");
        }
    }
    if (swapBanks)
    {
...
genFunction (iCode * ic)
{
    ...
    fc+=1000; // Function label prefix number
        ...
        for (i = 0; i < mcs51_nRegs; i++)
          {
            if (regs8051[i].type != REG_BIT)
        	  ...
        	  {
                  emitcode ("", "%s = 0x%02x", regs8051[i].dname, 8 * rbank + regs8051[i].offset);
        		  emitcode ("using", "%d", rbank);
        	  }
                else
                  emitcode ("", "%s = %s + 0x%02x",
...
#define MAX_REGISTER_BANKS 4
        ...
        if (!switchedpsw)
        {
            ...
            //emitcode ("using", "%d", FUNC_REGBANK (sym->type));
        }
    }
    else
    {
    ...
    if (IFFUNC_ISCRITICAL (ftype))
    {
        ...
        emitcode ("setb", "c");
        emitcode ("jbc", "ea,L%06d?", (tlbl->key + fc)); /* atomic test & clear */
        emitcode ("clr", "c");
        emitLabel (tlbl);
...
genEndFunction (iCode * ic)
{
    ...
    else
    {
        ...
        {
            emitcode ("ljmp", "__c51_banked_ret");
        }
        else
        {
            emitcode ("ret", "");
...
jumpret:
    ...
    if (!(ic->next && ic->next->op == LABEL &&
        ...
        emitcode ("ljmp", "L%06d?", (returnLabel->key + fc));
...
genGoto (iCode * ic)
{
    emitcode ("ljmp", "L%06d?", (IC_LABEL (ic)->key + fc));
}
...
genPlusIncr (iCode * ic)
{
            ...
            (icount == 1))
    {
        ...
        {
            emitcode ("cjne", "%s,#0x00,L%06d?", l, tlbl->key + fc);
        }
        else
        {
            emitcode ("clr", "a");
            emitcode ("cjne", "a,%s,L%06d?", l, tlbl->key + fc);
        }
        ...
        if (size > 2)
        {
            if (!strcmp(l, "acc"))
            {
                emitcode("jnz", "!tlabel", tlbl->key + fc);
            }
...
genPlusBits (iCode * ic)
{
    ...
    if (AOP_TYPE (IC_RESULT (ic)) == AOP_CRY)
    {
        ...
        emitcode ("jnb", "%s,L%06d?", AOP (IC_RIGHT (ic))->aopu.aop_dir, (lbl->key + fc));
        emitcode ("cpl", "c");
        emitLabel (lbl);
        outBitC (IC_RESULT (ic));
    }
...
genMinusBits (iCode * ic)
{
    ...
    else
    {
        emitcode ("mov", "c,%s", AOP (IC_RIGHT (ic))->aopu.aop_dir);
        emitcode ("subb", "a,acc");
        emitcode ("jnb", "%s,L%06d?", AOP (IC_LEFT (ic))->aopu.aop_dir, (lbl->key + fc));
        emitcode ("inc", "a");
        emitLabel (lbl);
        aopPut (IC_RESULT (ic), "a", 0);
                ...
                operand * result)
{
    ...
    else /* ! literal */
    {
        ...
        else
        {
            ...
            emitcode ("jnb", "acc.7,L%06d?", (lbl->key + fc));
    ...
    if (runtimeSign || compiletimeSign)
    {
        lbl = newiTempLabel (NULL);
        if (runtimeSign)
            emitcode ("jnb", "F0,L%06d?", (lbl->key + fc));
               ...
               operand * result)
{
    ...
    if (AOP_TYPE(right) == AOP_LIT)
    {
        ...
        case 128:
            ...
            /* see file "C51opt.c", function "convertToFcall()" */

            MOVA (aopGet (left, 0, FALSE, FALSE));
            emitcode ("mov", "c,acc.7");
            emitcode ("anl", "a,#0x%02x", val - 1);
            lbl = newiTempLabel (NULL);
            emitcode ("jz", "L%06d?", (lbl->key + fc));
            emitcode ("jnc", "L%06d?", (lbl->key + fc));
            ...
            if (size)
            {
                ...
                emitcode ("sjmp", "L%06d?", (lbl2->key + fc));
            }
            emitLabel (lbl);
            aopPut (result, "a", 0);
            while (size--)
...
genIfxJump (iCode * ic, char *jval, operand *left, operand *right, operand *result, iCode *popIc)
{
    ...
    if (strcmp (inst, "jb") == 0 || strcmp (inst, "jnb") == 0)
        emitcode (inst, "%s,L%06d?", jval, (tlbl->key + fc));
    else
        emitcode (inst, "L%06d?", tlbl->key + fc);
    freeForBranchAsmop (result);
    freeForBranchAsmop (right);
    freeForBranchAsmop (left);
    emitcode ("ljmp", "L%06d?", jlbl->key + fc);
    emitLabel (tlbl);
...
gencjneshort (operand * left, operand * right, symbol * lbl)
{
             ...
             (IS_AOP_PREG (left) && !IS_AOP_PREG (right)))
    {
        while (size--)
        {
            ...
            if ((AOP_TYPE (left) == AOP_DIR && AOP_TYPE (right) == AOP_LIT) &&
                ...
                emitcode ("jnz", "L%06d?", lbl->key + fc);
            else
                emitcode ("cjne", "a,%s,L%06d?",
    ...
    else
    {
        ...
        while (size--)
        {
            ...
            emitcode ("cjne", "a,b,L%06d?", lbl->key + fc); //JCF changed _B to b
            offset++;
        }
    }
}
...
gencjne (operand * left, operand * right, symbol * lbl, bool useCarry)
{
    ...
    if (useCarry)
        ...
        MOVA (one);
    emitcode ("sjmp", "L%06d?", tlbl->key + fc);
    emitLabel (lbl);
    if (useCarry)
        CLRC;
    else
    ...
    if (ifx && !AOP_SIZE (result))
    {
                ...
                ((AOP_TYPE (right) == AOP_CRY) || (AOP_TYPE (right) == AOP_LIT)))
        {
            ...
            else
            {
                symbol *lbl = newiTempLabel (NULL);
                emitcode ("mov", "c,%s", AOP (left)->aopu.aop_dir);
                emitcode ("jb", "%s,L%06d?", AOP (right)->aopu.aop_dir, (lbl->key + fc));
                emitcode ("cpl", "c");
                emitLabel (lbl);
            }
            /* if true label then we jump if condition
            ...
            if (IC_TRUE (ifx))
            {
                emitcode ("jnc", "L%06d?", tlbl->key + fc);
                freeForBranchAsmop (result);
                freeForBranchAsmop (right);
                freeForBranchAsmop (left);
                popForBranch (popIc, FALSE);
                emitcode ("ljmp", "L%06d?", IC_TRUE (ifx)->key + fc);
            }
            else
            {
                emitcode ("jc", "L%06d?", tlbl->key + fc);
                freeForBranchAsmop (result);
                freeForBranchAsmop (right);
                freeForBranchAsmop (left);
                popForBranch (popIc, FALSE);
                emitcode ("ljmp", "L%06d?", IC_FALSE (ifx)->key + fc);
            }
            emitLabel (tlbl);
        }
        else
        {
            ...
            {
                symbol *lbl = newiTempLabel (NULL);
                emitcode ("sjmp", "L%06d?", lbl->key + fc);
                emitLabel (tlbl);
                freeForBranchAsmop (result);
                freeForBranchAsmop (right);
                freeForBranchAsmop (left);
...
genOrOp (iCode * ic)
{
    ...
    else
    {
        tlbl = newiTempLabel (NULL);
        toBoolean (left);
        emitcode ("jnz", "L%06d?", tlbl->key + fc);
        toBoolean (right);
        emitLabel (tlbl);
        outBitAcc (result);
    }
...
continueIfTrue (iCode * ic, iCode * popIc)
{
	popForBranch (popIc, TRUE);
    if (IC_TRUE (ic))
        emitcode ("ljmp", "L%06d?", IC_TRUE (ic)->key + fc);
    ic->generated = 1;
}
...
jumpIfTrue (iCode * ic, iCode * popIc)
{
	popForBranch (popIc, TRUE);
    if (!IC_TRUE (ic))
        emitcode ("ljmp", "L%06d?", IC_FALSE (ic)->key + fc);
    ic->generated = 1;
}
...
jmpTrueOrFalse (iCode * ic, symbol * tlbl, operand *left, operand *right, operand *result, iCode * popIc)
{
    ...
    if (IC_TRUE (ic))
    {
        ...
        emitcode ("sjmp", "L%06d?", nlbl->key + fc);
        emitLabel (tlbl);
		popForBranch (popIc, FALSE);
        freeForBranchAsmop (result);
        freeForBranchAsmop (right);
            ...
            while (sizel--)
            {
                if ((bytelit = ((lit >> (offset * 8)) & 0x0FFL)) != 0x0L)
                {
                    ...
                    if ((posbit = isLiteralBit (bytelit)) != 0)
                        emitcode ("jb", "acc.%d,L%06d?", (posbit - 1) & 0x07, tlbl->key + fc);
                    else
                    {
                        if (bytelit != 0x0FFL)
                            emitcode ("anl", "a,%s",
        ...
        else
        {
            ...
            else
            {
                ...
                if ((AOP_TYPE (result) == AOP_CRY) && ifx)
                {
                    symbol *tlbl = newiTempLabel (NULL);
                    emitcode ("jb", "%s,L%06d?",
                              AOP (left)->aopu.aop_dir, tlbl->key + fc);
                    toBoolean (right);
                    emitcode ("jnz", "L%06d?", tlbl->key + fc);
                    jmpTrueOrFalse (ifx, tlbl, left, right, result, ic->next);
                    goto release;
...
genXor (iCode * ic, iCode * ifx)
{
    ...
    if (AOP_TYPE (left) == AOP_CRY)
    {
        ...
        else
        {
            ...
            emitcode ("jnb", "%s,L%06d?", AOP (left)->aopu.aop_dir, (tlbl->key + fc));
            emitcode ("cpl", "c");
            emitLabel (tlbl);
        }
        // bit = c
...
AccSRsh (int shCount)
{
    ...
    if (shCount != 0)
    {
        ...
        else
        {
            ...
            /* and kill the higher order bits */
            emitcode ("anl", "a,#0x%02x", SRMask[shCount]);
            emitcode ("jnb", "acc.%d,L%06d?", 7 - shCount, tlbl->key + fc);
...
genLeftShift (iCode * ic)
{
    ...
    if (size == 1)
    {
        ...
        l = aopGet (left, 0, FALSE, FALSE);
        MOVA (l);
        emitcode ("sjmp", "L%06d?", tlbl1->key + fc);
        emitLabel (tlbl);
        emitcode ("add", "a,acc");
        emitLabel (tlbl1);
        emitcode ("djnz", "b,L%06d?", tlbl->key + fc);
        popB (pushedB);
        aopPut (result, "a", 0);
        goto release;
    }
...
genUnpackBits (operand * result, char *rname, int ptype, iCode *ifx)
{
    ...
    if (blen < 8)
    {
        ...
        if (!SPEC_USIGN (etype))
        {
            ...
            emitcode ("jnb", "acc.%d,L%06d?", blen - 1, tlbl->key + fc);
    ...
    if (rlen)
    {
        ...
        if (!SPEC_USIGN (etype))
        {
            ...
            emitcode ("jnb", "acc.%d,L%06d?", rlen - 1, tlbl->key + fc);
...
genJumpTab (iCode * ic)
{
    symbol *jtab,*jtablo,*jtabhi,*temp1,*temp2;
    char *l;
    unsigned int count;
    ...
    if( count <= 16 )
    {
        ...
        emitcode ("mov", "dptr,#L%06d?", jtab->key + fc);
        emitcode ("jmp", "@a+dptr");
        emitLabel (jtab);
        /* now generate the jump labels */
        for (jtab = setFirstItem (IC_JTLABELS (ic)); jtab;
                jtab = setNextItem (IC_JTLABELS (ic)))
            emitcode ("ljmp", "L%06d?", jtab->key + fc);
    }
    else
    {
        ...
        temp1 = newiTempLabel (NULL);
        temp2 = newiTempLabel (NULL);
        ...
        {
            emitLabel (temp1);
            emitcode ("add", "a,#(L%06d?-3-L%06d?)", jtablo->key + fc, temp1->key + fc);
            emitcode ("movc", "a,@a+pc");
            emitcode ("push", "acc");

            MOVA (l);
            emitLabel (temp2);
            emitcode ("add", "a,#(L%06d?-3-L%06d?)", jtabhi->key + fc, temp2->key + fc);
            emitcode ("movc", "a,@a+pc");
            emitcode ("push", "acc");
        }
        else
        {
            /* this scales up to n<=255, but needs two more bytes and changes dptr */
            emitcode ("mov", "dptr,#L%06d?", jtablo->key + fc);
            emitcode ("movc", "a,@a+dptr");
            emitcode ("push", "acc");

            MOVA (l);
            emitcode ("mov", "dptr,#L%06d?", jtabhi->key + fc);
            emitcode ("movc", "a,@a+dptr");
            emitcode ("push", "acc");
        }
        ...
        for (jtab = setFirstItem (IC_JTLABELS (ic)); jtab;
                jtab = setNextItem (IC_JTLABELS (ic)))
            emitcode ("db", "L%06d?", jtab->key + fc);

        /* now generate jump table, MSB */
        emitLabel (jtabhi);
        for (jtab = setFirstItem (IC_JTLABELS (ic)); jtab;
                jtab = setNextItem (IC_JTLABELS (ic)))
            emitcode ("db", "L%06d?>>8", jtab->key + fc);
    }
}
...
genCast (iCode * ic)
{
  ...
  if (AOP_SIZE (result) <= AOP_SIZE (right) && !IS_BIT (operandType (result)))
    {
...
genDjnz (iCode * ic, iCode * ifx)
{
    ...
    {
        emitcode ("djnz", "%s,L%06d?", aopGet (IC_RESULT (ic), 0, FALSE, FALSE),
                  lbl->key + fc);
    }
    emitcode ("sjmp", "L%06d?", lbl1->key + fc);
    emitLabel (lbl);
    emitcode ("ljmp", "L%06d?", IC_TRUE (ifx)->key + fc);
    emitLabel (lbl1);
