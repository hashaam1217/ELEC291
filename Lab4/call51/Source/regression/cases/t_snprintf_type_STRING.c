...
#if defined(C51) && !defined(C51_ds390) && !(defined(C51_mcs51) && (defined(C51_USE_XSTACK) || defined(C51_MODEL_HUGE)))
  {1.0, "%f", "<NO FLOAT>"},
#else
  {1.0, "%f", "1.000000"},
  {1.96, "%3.1f", "2.0"},
...
__runSuite(void)
{
  __prints("Running test_snprintf\n");
  test_snprintf();
}
...
__getSuiteName(void)
{
  return "cases\\t_snprintf_type_STRING";
}
