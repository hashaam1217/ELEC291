/*-------------------------------------------------------------------------
  C51cse.c - source file for Common Subexpressions and other utility
...
algebraicOpts (iCode * ic, eBBlock * ebp)
{
    ...
    case '/':
        ...
        {
            ic->op = '=';
            if(IS_FLOAT (operandType (IC_RESULT (ic) ) ) )
                IC_RIGHT (ic) = operandFromLit (0x3f800000); // JCF: 1.0
            else
                IC_RIGHT (ic) = operandFromLit (1);
            IC_LEFT (ic) = NULL;
            IC_RESULT (ic) = operandFromOperand (IC_RESULT (ic));
...
constFold (iCode * ic, set * cseSet)
{
	...
	/*Patch for the nasty bug that happens when doing something like this: a=b-0.5-0.5;*/

	if ( ( (fabs( operandLitValue (IC_RIGHT (ic)) + operandLitValue (IC_RIGHT (dic)) )==1.0) ||
	       (fabs( operandLitValue (IC_RIGHT (ic)) - operandLitValue (IC_RIGHT (dic)) )==1.0) ) &&
		   (IS_FLOAT (operandType (IC_LEFT  (ic)))) ) return 0;
           ...
           ebbIndex * ebbi)
{
    ...
    for (ic = ebb->sch; ic; ic = ic->next)
    {
		...
		if (POINTER_SET (defic) && (IS_SYMOP (IC_RESULT (ic)) || IS_OP_LITERAL (IC_RESULT (ic))))
        {
			if (IS_SYMOP (IC_RESULT (ic)))
			{
