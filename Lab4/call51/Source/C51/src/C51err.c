...
} ErrTab [] =
{
    ...
    },
    {
        ...
        "C51 cannot pass structure '%s' as function argument"
    },
    {
    ...
    },
    {
        ...
        " please report problem and send source code at C51-USER list on SF.Net"
    },
    {
    ...
    {
        E_ISR_PROTOTYPE, ERROR_LEVEL_ERROR,
        "Interrupt Service Routine prototypes are not allowed"
    },
	{
		W_EMPTY_IF_STATEMENT, ERROR_LEVEL_WARNING,
		"'if/else' with empty statement."
	},
 	{
...
{
    if (_C51ERRG.out == NULL)
    {
        _C51ERRG.out = DEFAULT_ERROR_OUT;
    }

    if (ErrTab[errNum].errIndex != errNum)
    {
        fprintf(_C51ERRG.out,
                "Internal error: error table entry for %d inconsistent.", errNum);
    }

    if ((ErrTab[errNum].errType >= _C51ERRG.logLevel) && (!_C51ERRG.disabled[errNum]))
    {
        if ( ErrTab[errNum].errType == ERROR_LEVEL_ERROR || _C51ERRG.werror )
            fatalError++ ;

        if ( filename && lineno )
        {
            if(_C51ERRG.style)
                fprintf(_C51ERRG.out, "%s(%d) : ",filename,lineno);
            else
                fprintf(_C51ERRG.out, "%s:%d: ",filename,lineno);
        }
        else if (lineno)
        {
            fprintf(_C51ERRG.out, "at %d: ", lineno);
        }
        else
        {
            fprintf(_C51ERRG.out, "-:0: ");
        }

        switch(ErrTab[errNum].errType)
        {
        case ERROR_LEVEL_ERROR:
            fprintf(_C51ERRG.out, "error %d: ", errNum);
            break;
        ...
        case ERROR_LEVEL_PEDANTIC:
            if (_C51ERRG.werror)
                ...
                fprintf(_C51ERRG.out, "warning %d: ", errNum);
            break;
        case ERROR_LEVEL_INFO:
            fprintf(_C51ERRG.out, "info %d: ", errNum);
            break;
        default:
            break;
        }

        vfprintf(_C51ERRG.out, ErrTab[errNum].errText,marker);
        fprintf(_C51ERRG.out, "\n");
    }
    else
    {
