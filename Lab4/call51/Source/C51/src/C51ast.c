/*-------------------------------------------------------------------------
  C51ast.c - source file for parser support & all ast related routines
...
copyAstValues (ast * dest, ast * src)
{
    ...
    case SWITCH:
        ...
        dest->values.switchVals.swSuffix =
            src->values.switchVals.swSuffix;
        break;
...
resolveSymbols (ast * tree)
{
            ...
            !tree->opval.val->sym->implicit)
    {
		...
		{
			//printf("csym->name=%s\n", csym->name);
			if(strncmp(csym->name, "_str_", 5)==0)
			{
				csym->used++; // JCF: Solves problem with void *foo[]={(void *)&("X"[0])}; in gcc-torture-execute-921019-1.c
			}
		}
        ...
        if (csym && tree->opval.val->sym != csym)
        {
			...
			if (*csym->rname) //  fixes bug 3188899
			{
              ...
              bool rightmost)
{
    ...
    if (defParm && !*actParm)
    {
 	...
 	if (IS_AST_PARAM (*actParm)) // fixed bugs 3166064 and 3394118
 	{
 		(*actParm)->decorated = 1;
 		if ((*actParm)->reversed)
 		{
    ...
    *actParm = decorateType (*actParm, resultType);
 	  	 ...
 	  	 (compareType (defParm->type, (*actParm)->ftype) < 0) )
    {
        ...
        resultType = getResultTypeFromType (defParm->type); // Fixes bugs fixed bugs 3166064 and 3394118
...
createIvalArray (ast * sym, sym_link * type, initList * ilist, ast *rootValue)
{
    ...
    {
		/*JCF: Some optimization required here!*/
		//rast = newNode(INLINEASM, rast, NULL);
		//rast->values.inlineasm=";;;;;;;;;;;;;;;;;;;;;\n; LINE 1212 of C51ast.c\n;;;;;;;;;;;;;;;;;;;;;;\n";
		//printf("Symbol: %s\n", AST_SYMBOL(sym)->name);
        for (;;)
        {
            ast *aSym;
			...
			// If there is not an initializer for an array element, then do not initialize it!
			//if((iloop!=NULL)||(idx==0)) //disabled for now...
			{
				...
				//if(idx==0)
				//{
				//	rast = newNode(INLINEASM, sym, NULL);
				//	rast->values.inlineasm="; This should be only after the first element.\n";
				//}
				//printf("Symbol: %s\n", AST_SYMBOL(rootValue)->name);
				//if(SPEC_SCLS(TETYPE(aSym)) == S_XDATA)
				//{
				//	printf("Initializing some xdata array...\n");
				//}
 				
				/* JCF: For testing only */
				/*printf("[%d]=%d\n", 
					(int) ulFromVal (aSym->right->opval.val),
					(int) ulFromVal (iloop->init.node->opval.val) );*/
			}
...
stringToSymbol (value * val)
{
    ...
    strncpyz (sym->rname, name, C51_NAME_MAX);
...
constExprValue (ast * cexpr, int check)
{
    ...
    if (!IS_LITERAL (cexpr->ftype))
    {
                ...
                IS_ARRAY (cexpr->ftype))
        {
            ...
            strncpyz (val->name, cexpr->opval.val->sym->rname, C51_NAME_MAX);
            return val;
        }
...
decorateType (ast * tree, RESULT_TYPE resultType)
{
    ...
    case '/':
		...
		// This optimization is disabled because sometimes it fails.
#if 0
        /* if right is a literal and */
    ...
    case '+':
       ...
       /* this is not a unary operation */
		...
		if(IS_PTR (LTYPE (tree)) && IS_CHAR (RTYPE (tree)))
		{
			if(!IS_UNSIGNED(RTYPE (tree)))
			{
				if (SPEC_SCLS (LETYPE (tree)) == S_XDATA)
				{
					// printf("xdata_pointer + signed_char: adding cast to integer.\n"); // Debug JCF
					tree->right = addCast (tree->right, RESULT_TYPE_INT, TRUE);
				}
				else if (SPEC_SCLS (LETYPE (tree)) == S_CODE)
				{
					// printf("code_pointer + signed_char: adding cast to integer.\n"); // Debug JCF
					tree->right = addCast (tree->right, RESULT_TYPE_INT, TRUE);
				}
			}
		}
    ...
    case '-':                   /* can be unary   */
		...
		/* Ok, if the user is writing code like this, well it doesn't make sense,
		but at least we are subtrating the right values.*/
		if(IS_CHAR (LTYPE (tree)) && IS_PTR (RTYPE (tree)))
		{
			if(!IS_UNSIGNED(LTYPE (tree)))
			{
				if (SPEC_SCLS (RETYPE (tree)) == S_XDATA)
				{
					// printf("signed_char - xdata_pointer: adding cast to integer.\n"); // Debug JCF
					tree->left = addCast (tree->left, RESULT_TYPE_INT, TRUE);
				}
				else if (SPEC_SCLS (LETYPE (tree)) == S_CODE)
				{
					// printf("signed_char - code_pointer : adding cast to integer.\n"); // Debug JCF
					tree->left = addCast (tree->left, RESULT_TYPE_INT, TRUE);
				}
			}
		}
    ...
    case CALL:
                ...
                !tree->right->decorated)
        {
			...
			TTYPE (tree) = copyLinkChain(functype->next); //  Fixes bug 3419769
            TETYPE (tree) = getSpec (TTYPE (tree));
			SPEC_SCLS (TETYPE (tree)) = S_FIXED; // Fixes bug 3419769
        }
        return tree;
...
createFunction (symbol * name, ast * body)
{
    ...
    if (getenv("C51_DEBUG_FUNCTION_POINTERS"))
        fprintf (stderr, "C51ast.c:createFunction(%s)\n", name->name);
...
#define INDENT(x,f) { fprintf (f, "%s:%d:", tree->filename, tree->lineno); fprintf(f, "%*s", (x)&0xff, ""); }
/*-----------------------------------------------------------------*/
...
void ast_print (ast * tree, FILE *outfile, int indent)
{
    ...
    if (tree->opval.op == BLOCK)
    {
        ...
        while (decls)
        {
            ...
            fprintf (outfile, "DECLARE SYMBOL (L%d B%d %s=%p) type (",
                     decls->level, decls->block, decls->name, decls);
