...
#define XPFILE1 "c:/usr/local/lib/bison.hairy"
	...
	#include <stdio.h>
	#include "bison_hairy.h"
	#include "bison_simple.h"
	#define unlink _unlink
	char XPFILE[PATH_MAX]= "c:/usr/local/lib/bison.simple";
	char XPFILE1[PATH_MAX]="c:/usr/local/lib/bison.hairy";
	FILE * BISON_SIMPLE = NULL;
	FILE * BISON_HAIRY  = NULL;
#endif
...
#endif /* MSDOS */
	  /*JCF: create our own BISON.SIMPLE file here*/
#if defined (_MSC_VER)
		tmp_base = getenv ("TEMP");		/* Windows95 defines this ... */
			if (tmp_base == 0)
		tmp_base = getenv ("Temp");		/* ... while NT prefers this */
		...
		sprintf(XPFILE, "%s\\bison.simple", tmp_base);
		BISON_SIMPLE=fopen(XPFILE, "wb");
		fwrite(bison_simple, 1, strlen(bison_simple), BISON_SIMPLE);
		fclose(BISON_SIMPLE);
...
#endif
	  /*JCF: create your own BISON.HAIRY file here*/
#if defined (_MSC_VER)
		tmp_base = getenv ("TEMP");		/* Windows95 defines this ... */
			if (tmp_base == 0)
		tmp_base = getenv ("Temp");		/* ... while NT prefers this */
		...
		sprintf(XPFILE1, "%s\\bison.hairy", tmp_base);
		BISON_HAIRY=fopen(XPFILE1, "wb");
		fwrite(bison_hairy, 1, strlen(bison_hairy), BISON_HAIRY);
		fclose(BISON_HAIRY);
		filename=XPFILE1;
#endif
