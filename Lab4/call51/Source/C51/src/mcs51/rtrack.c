...
/*-------------------------------------------------------------------------
  Status:
    ...
    - only active if environment variable C51_RTRACK is set
...
void rtrackUpdate (const char *line)
{
    ...
    {
        if (!strncmp (line+4,"acc",3))
        {
            REGS8051_UNSET (A_IDX);
            return;
        }
        if (!strncmp (line+4,"ar2",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (2));
            return;
        }
        if (!strncmp (line+4,"ar3",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (3));
            return;
        }
        if (!strncmp (line+4,"ar4",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (4));
            return;
        }
        if (!strncmp (line+4,"ar5",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (5));
            return;
        }
        if (!strncmp (line+4,"ar6",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (6));
            return;
        }
        if (!strncmp (line+4,"ar7",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (7));
            return;
        }
        if (!strncmp (line+4,"ar0",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (0));
            return;
        }
        if (!strncmp (line+4,"ar1",3))
        {
            REGS8051_UNSET (Rx_NUM_TO_IDX (1));
            return;
        }
    }
