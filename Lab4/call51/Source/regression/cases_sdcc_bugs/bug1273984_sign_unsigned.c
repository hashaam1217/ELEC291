...
void fooInt(unsigned int val) {
  ASSERT(val==3);
}

void fooChar(unsigned char val) {
  ASSERT(val==6);
}
...
{
  volatile unsigned char charVal=3;
  volatile unsigned int intVal=0x4040;
...
__getSuiteName(void)
{
  return "cases_sdcc_bugs\\bug1273984_sign_unsigned";
}
