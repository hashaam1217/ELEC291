...
/* Right.  What are the parts of the C stream?  From C51.lex:
   D = [0..9]           digits
    ...
    *p = '\0';
    ...
    else
    {
        ...
        if (port->process_pragma && !port->process_pragma (line))
            return;
        /* PENDING: all the C51 shared pragmas */
        /* Nothing handled it */
...
{
    /* These conditions are ripped directly from C51.lex */
