/*-------------------------------------------------------------------------
  C51opt.c - calls all the optimizations routines and does some of the
...
cnvToFloatCast (iCode * ic, eBBlock * ebp)
{
    ...
    /* remove it from the iCode */
    remiCodeFromeBBlock (ebp, ic);
	if (IS_SYMOP (IC_RIGHT (ic))) bitVectUnSetBit (OP_USES (IC_RIGHT (ic)), ic->key);
	if (IS_SYMOP (IC_RESULT (ic))) bitVectUnSetBit (OP_DEFS (IC_RESULT (ic)), ic->key);
