/*-------------------------------------------------------------------------
  C51peeph.c - The peep hole optimizer: for interpreting the
                peep hole rules
...
FBYNAME (labelIsUncondJump)
{
    ...
    for (pl = currPl; pl; pl = pl->next)
    {
        ...
        {
            if (strncmp(pl->line, label, len) == 0)
                break; /* Found Label.  Labels look like L001001?:*/
            if (strlen(pl->line) != 9       || (*(pl->line)!='L')   ||
                    !ISCHARDIGIT(*(pl->line+1)) || !ISCHARDIGIT(*(pl->line+2)) ||
                    !ISCHARDIGIT(*(pl->line+3)) || !ISCHARDIGIT(*(pl->line+4)) ||
                    !ISCHARDIGIT(*(pl->line+5)) || !ISCHARDIGIT(*(pl->line+6)) ||
