...
void foo (struct E *screen, unsigned int c, int columns, struct B *row)
{
  ...
  int i;
  col = screen->col;
  /*attr = screen->defaults.attr; struct assignment not yet supported by C51 */
  memcpy (&attr, &(screen->defaults.attr), sizeof (struct D));
  attr.columns = columns;
  row->cells->data[col].c = c;
  /*row->cells->data[col].attr = attr; struct assignment not yet supported by C51 */
...
testTortureExecute (void)
{
  ...
  memset (&c, 0, sizeof c);
  foo (&e, 65, 2, &b);
  /*d = e.defaults.attr; struct assignment not yet supported by C51 */
...
__getSuiteName(void)
{
  return "cases_gcc\\gcc-torture-execute-pr39339";
}
