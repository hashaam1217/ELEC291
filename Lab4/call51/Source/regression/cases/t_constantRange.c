...
/* on 32bit host: -1 is presented as 32 bit int, 16 bit unsigned short is promoted to 32 bit int */
  ...
  ASSERT (! (         0 - 1  >= u16)); /* !(-1 <= 0) */
#else
/* on 16bit c51: int (-1) is promoted to unsigned int (0xffff) */
  ASSERT (  (         0 - 1  >  u16)); /* 0xffff > 0 */
  ASSERT (! (         0 - 1  <= u16)); /* !(0xffff <= 0) */
#endif

   /* c51 can't hold a number (INT32_MIN - 1) or (INT32_MAX + 1),
