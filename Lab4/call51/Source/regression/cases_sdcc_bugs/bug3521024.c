...
struct cvu_huffman_state
{
	...
	unsigned char mybit;	/* Position of currently processed mybit */
...
const unsigned char mydata[] = {72, 60, 102, 102, 123, 15};
const unsigned char umydata[] = {0x01, 0x02, 0x03, 0x04, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x0d, 0x0e, 0x0f};

unsigned char get_mydata(void)
{
  static int i;
  return(mydata[i++]);
}

unsigned char get_mydata2(void)
{
  static int i;
  return(mydata[i++]);
}
...
void testBug(void)
{
  ...
  init_huffman(&state, &get_mydata, huffman_tree, HUFFMAN_ROOT, HUFFMAN_LS, HUFFMAN_BS, HUFFMAN_RS);

  for(i = 0; i < 15; i++)
    ASSERT(huffman_recursive(&state) == umydata[i]);

  init_huffman(&state, &get_mydata2, huffman_tree, HUFFMAN_ROOT, HUFFMAN_LS, HUFFMAN_BS, HUFFMAN_RS);

  for(i = 0; i < 15; i++)
    ASSERT(huffman_iterative(&state) == umydata[i]);
}
...
__getSuiteName(void)
{
  return "cases_sdcc_bugs\\bug3521024";
}
